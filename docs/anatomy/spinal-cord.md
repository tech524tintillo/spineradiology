# Spinal Cord

## Definition

The spinal cord is the major conduit of the **central nervous system** extending from the brainstem to the conus medullaris, situated within the vertebral (spinal) canal. In adults, the spinal cord typically terminates at the level of **L1–L2** (conus medullaris). It is approximately 45 cm long, roughly 1 cm in diameter, and is surrounded by three meningeal layers and cerebrospinal fluid (CSF).

## Anatomy

### External Structure

The spinal cord has two prominent **enlargements** corresponding to the nerve supply of the limbs:

| Enlargement | Levels | Function |
|-------------|--------|----------|
| **Cervical enlargement** | C4–T1 | Brachial plexus — upper extremity innervation |
| **Lumbar enlargement** | L1–S2 | Lumbosacral plexus — lower extremity innervation |

Key external features include:

- **Anterior median fissure** — deep groove along the anterior midline
- **Posterior median sulcus** — shallow groove along the posterior midline
- **Anterolateral sulcus** — exit point of ventral (motor) nerve roots
- **Posterolateral sulcus** — entry point of dorsal (sensory) nerve roots

### Internal Structure (Cross-Section)

The spinal cord in cross-section demonstrates a characteristic butterfly-shaped gray matter core surrounded by white matter:

**Gray matter (central):**

- **Anterior horns** — contain lower motor neurons (alpha motor neurons)
- **Posterior horns** — receive sensory input from dorsal nerve roots
- **Lateral horns** — present at T1–L2 (sympathetic) and S2–S4 (parasympathetic); contain autonomic neurons
- **Central canal** — CSF-filled remnant canal in the center

**White matter (peripheral):**

- **Dorsal columns** — carry proprioception and fine touch (fasciculus gracilis, fasciculus cuneatus)
- **Lateral corticospinal tracts** — carry voluntary motor signals
- **Spinothalamic tracts** — carry pain and temperature sensation

![Spinal cord cross-section](https://upload.wikimedia.org/wikipedia/commons/thumb/d/dc/Medulla_spinalis_-_Section_-_English.svg/450px-Medulla_spinalis_-_Section_-_English.svg.png)

### Meningeal Coverings

Three meningeal layers surround the spinal cord:

| Layer | Description | Space Below |
|-------|-------------|------------|
| **Dura mater** | Tough outer layer; forms the thecal sac extending to S2 | Epidural space (fat, veins) |
| **Arachnoid mater** | Thin, avascular middle layer | Subarachnoid space (CSF) |
| **Pia mater** | Delicate layer adherent to the cord surface | — |

### Vascular Supply

- **Anterior spinal artery** (single) — supplies the anterior two-thirds of the cord
- **Posterior spinal arteries** (paired) — supply the posterior one-third of the cord
- **Segmental radicular arteries** — reinforce the spinal arteries; the largest is the **artery of Adamkiewicz** (typically T9–T12 on the left)

!!! tip "Clinical Pearl"
    The **anterior spinal artery** supplies the anterior two-thirds of the cord, including the corticospinal tracts and spinothalamic tracts. Occlusion causes **anterior cord syndrome** — motor paralysis and loss of pain/temperature sensation with preserved posterior column function (proprioception, vibration). See [Anterior Spinal Artery Syndrome](../vascular/anterior-spinal-artery-syndrome.md).

## Imaging Findings

### MRI

MRI is the definitive imaging modality for the spinal cord:

| Finding | T1 Signal | T2 Signal | Significance |
|---------|-----------|-----------|--------------|
| Normal cord | Intermediate (homogeneous) | Intermediate (homogeneous) | Smooth contours, uniform caliber |
| Cord edema/myelopathy | Low | **High** | Gliosis, demyelination, ischemia |
| Cord hemorrhage (acute) | Isointense | Low (deoxyhemoglobin) | Trauma, vascular malformation |
| Cord hemorrhage (subacute) | **High** (methemoglobin) | High | Evolving hemorrhage |
| Syrinx | **Dark** (CSF signal) | **Bright** (CSF signal) | Smooth, well-defined intramedullary cavity |
| Cord tumor | Variable | High | Expansion of cord, may enhance with contrast |
| Cord atrophy | — | — | Reduced caliber, may follow chronic myelopathy |

!!! note "Normal Cord Diameter"
    The normal spinal cord measures approximately **8–10 mm** in AP diameter at the cervical enlargement. Cord diameters less than 7 mm may indicate atrophy. The cord should have a smooth, symmetric contour on sagittal and axial images.

### CT Myelography

When MRI is contraindicated, CT myelography can evaluate:

- Cord compression by disc, osteophyte, or mass
- Cord atrophy
- Intradural pathology outlined by contrast

## Key Points

- The spinal cord extends from the brainstem to approximately L1–L2 (conus medullaris)
- Two enlargements (cervical and lumbar) correspond to limb innervation
- Gray matter (central) contains motor and sensory neurons; white matter (peripheral) contains ascending and descending tracts
- The anterior spinal artery supplies the anterior two-thirds of the cord
- MRI is the gold standard — increased T2 signal in the cord indicates myelopathy
- Normal cord AP diameter is 8–10 mm at the cervical enlargement

## Related Articles

- [Vertebral Column Overview](vertebral-column-overview.md)
- [Conus Medullaris and Cauda Equina](conus-cauda-equina.md)
- [Spinal Nerve Roots and Dermatomes](nerve-roots-dermatomes.md)
- [Spinal Arterial Supply](spinal-arterial-supply.md)
- [Syringomyelia](../congenital-developmental/syringomyelia.md)
- [Spinal Cord Injury Imaging](../trauma/spinal-cord-injury.md)
- [Anterior Spinal Artery Syndrome](../vascular/anterior-spinal-artery-syndrome.md)
- [Myelopathy Imaging Correlation](../special-topics/myelopathy.md)
